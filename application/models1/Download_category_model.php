...
class Download_category_model extends CI_Model {
    ...
    function get_public_download_category() {
        $query = $this->db->query(" SELECT * FROM {$this->download_category} ORDER BY category_id DESC ");
        return $query->result();
    }
    function get_download_category_by_id($id) {
        $id = $this->db->escape_str($id);
        $query = $this->db->query(" SELECT * FROM {$this->download_category} WHERE category_id = '$id' ");
        return (object)$query->row();
    }
    function insert_download_category($data) {
        $data = $this->master_model->escape_all($data);
        $this->db->insert($this->download_category, $data);
    ...
    function update_download_category($data, $id) {
        ...
        $this->db->update($this->download_category, $data);
        return $this->db->affected_rows();
    }
    function delete_download_category($id) {
        $id = $this->db->escape_str($id);
        $this->db->where('category_id', $id);
        $this->db->delete($this->download_category);
        return $this->db->affected_rows();
    }
    function get_download_category_all() {
        $query = $this->db->query(" SELECT * FROM {$this->download_category} ORDER BY category ASC ");
        return $query->result();
    }
    ...
    function get_download_category() {
        ...
        $sql = " SELECT * FROM {$this->download_category} $sql_where ";
