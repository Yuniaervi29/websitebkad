 ...
 $('input[type="checkbox"]').on('change', function() {
    ...
    function proses(id){
        $.ajax({
            type: "post",
            dataType: "json",
            url: "<?=base_url()?>admin/permohonan_penyalahgunaan_wewenang/get_data",
            data: ({id}),
   ...
   function hapus(id){
          ...
          Swal.fire({
            ...
            if (result.value) {
              $.ajax({
                        type : "POST",
                        url  : "<?php echo site_url('admin/permohonan_penyalahgunaan_wewenang/delete')?>",
                        dataType : "JSON",
                        data : {id:id},
                        success: function(){
                          $("#"+id).remove();
                          Swal.fire(
                            ...
                            'success'
                              ...
                              if(value){
                                 location.href= '<?php echo site_url('admin/register_penyalahgunaan_wewenang')?>';
                              }
                          });
                        },error:function(){
        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active">Register Penyalahgunaan Wewenang</li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
              ...
              <div class="card-body">
                <table id="example1" class="table table-bordered table-striped">
                  ...
                  <tbody>
                    <?php 
                    $no=0;
                    $isi = $this->db->query("SELECT a.*,b.status_id FROM inbox_penyalahgunaan a 
                        LEFT JOIN proses_penyalahgunaan_wewenang b
                        ON a.noreg=b.noreg
                    ...
                    foreach($isi->result() as $row){
                        ...
                        <button class='btn btn-danger btn-sm' onclick='hapus($row->id)'>
                          ...
                          <a href='".base_url()."admin/proses_penyalahgunaan_wewenang/cetak?no=$row->noreg' target='_blank' class='btn btn-primary'>
<span class='fa fa-print'></span> Print</a>
                        ...
                        <button class='btn btn-danger btn-sm' onclick='hapus($row->id)'>
                          ...
                          <a href='".base_url()."admin/proses_penyalahgunaan_wewenang/cetak?no=$row->noreg' target='_blank' class='btn btn-primary disabled'>
<span class='fa fa-print'></span> Print</a>
                    </td>
                    </tr>";
                        }
