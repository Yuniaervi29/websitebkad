...
class album_model extends CI_Model {
    ...
    function get_album_all() {
        $query = $this->db->query("SELECT a.* FROM album a  ");
        return $query->result();
    }
    ...
    function get_album_all_admin() {
        $query = $this->db->query(" SELECT * FROM {$this->album} ORDER BY album_title ASC ");
        return $query->result();
    }
    function get_album_select_admin() {
        $tmp = array();
        $res = $this->get_album_all_admin();
        if (count($res) > 0) {
            foreach ($res as $row) {
                $tmp[$row->album_id] = ucfirst($row->album_title);
            }
