...
class Prosedur_keberatan extends CI_Controller {
	...
	function index()
	{
        $post = $this->post_model->get_prosedur_keberatan();
        $output['POST'] = $post['result'];
		$output['PAGE_TITLE'] = 'PROSEDUR PENGAJUAN KEBERATAN';
		$this->admin->view('admin/prosedur_keberatan', $output);
    }

    function json()
	{
		$article = $this->post_model->get_article();
		$tmp = array();
    ...
    function add()
	{
        if (!isset($_POST['post_title']))
		{
		    ...
		    $this->admin->view('admin/prosedur_keberatan_form', $output);
		}else
		{
			...
			$data['post_status']	= '';
			$data['post_type']		= 'prosedur_keberatan';
