...
class Arsip extends CI_Controller {
	...
	function index()
	{
        $post = $this->post_model->get_arsip();
        $output['POST'] = $post['result'];
		$output['PAGE_TITLE'] = 'Arsip';
		$this->admin->view('admin/arsip', $output);
    }

	function json()
	{
		$arsip = $this->post_model->get_arsip();
		$tmp = array();
		$tmp['total'] = $arsip['num_rows'];
		$tmp['rows'] = $arsip['result'];
	...
	function add()
	{
        if (!isset($_POST['post_title']))
		{
		    ...
		    $this->admin->view('admin/arsip_form', $output);
		}
		else
		{
			...
			if( isset($_FILES['post_image']['tmp_name']) AND is_uploaded_file($_FILES['post_image']['tmp_name']) ){
				...
				$filename				= 'arsip_'.date('ymd_his_').rand(1111,9999).'_'.$filename;
	...
	function edit()
	{
        ...
        if (!isset($_POST['post_title']))
		{
		    ...
		    $this->admin->view('admin/article_form', $output);
