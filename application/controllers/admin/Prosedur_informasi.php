...
class Prosedur_informasi extends CI_Controller {
	...
	function index()
	{
        $post = $this->post_model->get_prosedur_informasi();
        $output['POST'] = $post['result'];
		$output['PAGE_TITLE'] = 'PROSEDUR PENGAJUAN INFORMASI';
		$this->admin->view('admin/prosedur_informasi', $output);
    }
    ...
    function add()
	{
        if (!isset($_POST['post_title']))
		{
		    ...
		    $this->admin->view('admin/prosedur_informasi_form', $output);
		}else
		{
			...
			$data['post_status']	= '';
			$data['post_type']		= 'prosedur_informasi';
